...
class SynchronousServer(object):
    def __init__(self, data_dir_path, config_file_path, port):
    ...
    def process_data_feeds(self, data_feeds, should_record = False):
        index = 1
        ...
        while index < len(data_feeds):
            ...
            if (((rgb_cam_type == 'RGB') & (semseg_cam_type == 'SS')) & (rgb_cam_num == semseg_cam_num) & (rgb_frame.frame == semseg_frame.frame)):
                ...
                #     cv2.imwrite(os.path.join(self.track_img_dir, 'cam_{}_frame_{}_rgb.jpg'.format(rgb_cam_num, rgb_frame.frame)), rgb_image)
                ...
                #     semseg_image = ClientSideBoundingBoxes.process_img(semseg_frame, self.view_width, self.view_height)
                    # cv2.imwrite(os.path.join(self.track_img_dir, 'cam_{}_frame_{}_semseg.jpg'.format(rgb_cam_num, rgb_frame.frame)), semseg_image)
    ...
    def save_track(self):
        ...
        log_file_path = os.path.join(self.log_dir_path, self.track_id + '.csv')
    ...
    def game_loop(self, track_id, track):
        ...
        def get_walker_control(current_location, next_location, speed):
            control = carla.WalkerControl()
            x_diff = next_location.x - current_location.x
            y_diff = next_location.y - current_location.y


            control.direction.x = x_diff
            control.direction.y = y_diff
                ...
                if recorded_frame > 14:
                    ...
                    if num_of_points_passed == -1:
                        ...
                        if not failed:
                            ...
                            num_of_points_passed += 1
                            current_location = pedes_model.get_location()
                            pedestrian_height = current_location.z

                            next_location = carla.Location(track[0][0], track[0][1], pedestrian_height)
                            # ai_walker_model.go_to_location(next_location)
                            # ai_walker_model.set_max_speed(track[0][2])

                            control = get_walker_control(current_location, next_location, track[0][2])
                            pedes_model.apply_control(control)
