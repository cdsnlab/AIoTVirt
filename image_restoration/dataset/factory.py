...
def create_unit_dataset(config, task: str, split: str, mode: str, dset_size: int = None,
                        image_augmentation: bool = True, shuffle: bool = True, cache: str = None) -> IRUnitDataset:
...
def create_dataset(config, split: str, mode: str, tasks: List[str] = None, dset_size: int = None,
                   verbose=True) -> IRDataset:
    """
    ...
    datasets = [
        create_unit_dataset(config, task, split=split, dset_size=None, image_augmentation=config.image_augmentation,
                            mode=mode)
...
def get_train_dataloader(config, split: str = 'train', pin_memory=True, verbose=True):
    ...
    if config.ddp is True:
        ...
        loader = DataLoader(dataset, batch_size=config.batch_size // torch.cuda.device_count(), shuffle=False,
                            pin_memory=pin_memory, drop_last=True, num_workers=config.num_workers, sampler=sampler)
    else:
        loader = DataLoader(dataset, batch_size=config.batch_size // torch.cuda.device_count(), shuffle=False,
                            pin_memory=pin_memory, drop_last=True, num_workers=config.num_workers)
    loader = DataLoader(dataset, batch_size=config.batch_size // torch.cuda.device_count(), shuffle=False,
                        pin_memory=pin_memory, drop_last=True, num_workers=config.num_workers)
    if verbose:
...
def get_finetune_dataloader(config, task: str, mode: str = 'resize', split: str = 'train', support_idx: int = 0,
                            pin_memory=True, cache: str = 'mem', verbose=True):
    # if config.no_eval:
    dset_size = config.n_steps * config.batch_size
    # else:
    # dset_size = config.val_iter*config.batch_size
    dset = create_unit_dataset(config, task=task, split=split, mode=mode, dset_size=config.shot,
                               image_augmentation=False, shuffle=False, cache=cache)
    dataset = IRFinetuneDataset(dset, shot=config.shot, support_idx=support_idx, dset_size=dset_size,
                                precision=config.precision, shuffle_idx=False)
    # --- Distributed Data Parallel initialize --- #
    if config.ddp is True:
        sampler = DistributedSampler(dataset=dataset, shuffle=False)
        loader = DataLoader(dataset, batch_size=config.batch_size // torch.cuda.device_count(), shuffle=False,
...
def get_eval_dataloader(config, task: str, mode: str = 'resize', split: str = 'val', pin_memory=True, verbose=True):
    ...
    loader = DataLoader(dataset, batch_size=1, shuffle=False, pin_memory=pin_memory, drop_last=True,
...
def get_val_dataloaders(config, task: str = None, support_data=None, pin_memory=True, verbose=True):
    ...
    if config.stage == 0:
        if config.ddp is True and dist.get_rank() != 0:
            ...
            dataloaders = {
                task_name: get_eval_dataloader(config, task_name, split='val', mode='center_crop',
...
def get_support_data(config, task: str, split: str = 'shots', support_idx: int = 0, pin_memory=True, verbose=True):
    dset = create_unit_dataset(config, task=task, split=split, mode='resize', dset_size=config.shot,
                               image_augmentation=False, shuffle=False)
    dataset = IRFinetuneDataset(dset, shot=config.shot, support_idx=support_idx, dset_size=config.shot,
                                precision=config.precision, shuffle=False)
...
def generate_support_data(config, data_path: str, split: str = 'shots', support_idx: int = 0, verbose: bool = True):
    ...
    for task in tasks:
        ...
        dataset = create_unit_dataset(config, task=task, split=split, mode='center_crop', dset_size=None,
