...
class MetaWeatherDecoder(nn.Module):
    def __init__(self, num_decoders: List = [1, 1, 1, 1], dim=[1024, 1024, 512, 256], mode='swin'):
        ...
        self.blocks = nn.ModuleList([
            MetaWeatherDecoderBlock(n, d, upsample=upsample_from <= i < self.num_levels - 1) for i, (n, d) in
            enumerate(zip(num_decoders, dim))
        ])
...
class MetaWeather(nn.Module):
    def __init__(self, config, swin_config, n_tasks,
        ...
        if config.enc == 'swin':
            ...
            self.project_out = nn.Conv2d(in_channels=4, out_channels=3, kernel_size=3, padding=1, stride=1, groups=1,
                                         bias=True)

            self.refine_naf = NAFBlock(4)
            self.refine_conv = nn.Conv2d(in_channels=3, out_channels=3, kernel_size=3, padding=1, stride=1, groups=1,
                                         bias=True)
