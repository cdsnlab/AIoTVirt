...
class DetectionLayer(nn.Module):
    ...
    def forward(self, x, cuda, id):
        ...
        #----- important!
        if cuda: # had to fix cuda() --> to("cuda:1") in darknet.py but needs more fixing depending on application. 
            #x_offset = x_offset.to("cuda:0")
            #y_offset = y_offset.to("cuda:0")
            ...
            if int(id) % 2 == 0:
                ...
                x_offset = x_offset.to("cuda:1")
                y_offset = y_offset.to("cuda:1")
...
class Darknet(nn.Module):
    ...
    def forward(self, x, cuda, id):
        ...
        if cuda:
            #detections = detections.to("cuda:0")    
            if int(id) % 2 == 0:
                detections = detections.to("cuda:0")    
            else:
                detections = detections.to("cuda:1")
