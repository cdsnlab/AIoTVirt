...
from .Baseline_arch import  BaselineBlock

from einops import rearrange
from model.Restormer.reshape import from_4d_to_6d, from_6d_to_4d, parse_BTN
from model.Restormer.Restormer import grad_reverse
from model.matching_modules import ChannelMatchingModule, BasicCosineMatchingModule
    ...
    def __init__(self, img_channel=3, width=16, middle_blk_num=1, enc_blk_nums=[]):
        ...
        for num in enc_blk_nums:
            self.encoders.append(
                nn.Sequential(
                    *[BaselineBlock(chan) for _ in range(num)]
                )
            )
            self.downs.append(
        ...
        self.middle_blks = \
            nn.Sequential(
                *[BaselineBlock(chan) for _ in range(middle_blk_num)]
            )
...
class Baseline_Fewshot(nn.Module):
    def __init__(self, img_channel=3, width=32, middle_blk_num=1, 
        ...
        self.encoder = Baseline_Encoder(img_channel=img_channel, width=width, middle_blk_num=middle_blk_num, enc_blk_nums=enc_blk_nums)
        self.encoder_label = Baseline_Encoder(img_channel=img_channel, width=width, middle_blk_num=middle_blk_num, enc_blk_nums=enc_blk_nums)
        self.decoder = Baseline_Decoder(img_channel=img_channel, width=width, dec_blk_nums=dec_blk_nums)
