...
from models.vit.swin_modularized import SwinTransformerModularized
...
def load_pretrained(config, model: SwinTransformerModularized, logger, n_tasks=0):
    ...
    if n_tasks > 0:
        params = model.bias_parameter_names()
        for p in params:
            checkpoint_model[p] = repeat(checkpoint_model[p], '... -> T ...', T=n_tasks).contiguous()
