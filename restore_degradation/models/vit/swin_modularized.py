...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., n_tasks=0):
        ...
        self.proj = LinearModularized(n_tasks, dim, dim)
        self.proj_drop = nn.Dropout(proj_drop)

        trunc_normal_(self.relative_position_bias_table, std=.02)
        self.softmax = nn.Softmax(dim=-1)

    def forward(self, x, mask=None, t_idx=None):
        """
        Args:
...
class SwinTransformerBlockModularized(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 act_layer=nn.GELU, norm_layer=LayerNormModularized, n_tasks=0):
        super().__init__()
        self.dim = dim
        ...
        self.attn = WindowAttention(
            ...
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, n_tasks=n_tasks)
    ...
    def forward(self, x, t_idx=None):
        ...
        x = x + self.drop_path(self.mlp(self.norm2(x, t_idx=t_idx), t_idx=t_idx))
...
class BasicLayerModularized(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 drop_path=0., norm_layer=LayerNormModularized, downsample=None, use_checkpoint=False, n_tasks=0):
    ...
    def forward(self, x, t_idx=None):
        for blk in self.blocks:
            if self.use_checkpoint:
                x = checkpoint.checkpoint(blk, x, t_idx)
            else:
                x = blk(x, t_idx=t_idx)
...
class SwinTransformerModularized(nn.Module):
    r""" Swin Transformer
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 norm_layer=LayerNormModularized, ape=False, patch_norm=True,
                 use_checkpoint=False, n_tasks=0, **kwargs):
        super().__init__()
    ...
    def forward_features(self, x, t_idx=None):
        ...
        # import pdb; pdb.set_trace()
        x = self.norm(x, t_idx=t_idx)  # B L C
    ...
    def forward(self, x, t_idx=None):
        x, xlist = self.forward_features(x, t_idx=t_idx)
        # x = self.head(x)
        # return x, xlist
        # return xlist[:-1] + [x]
...
def build_swin_modularized(config, n_tasks=0):
    model = SwinTransformerModularized(
        ...
        norm_layer=partial(LayerNormModularized, eps=1e-6),
        n_tasks=n_tasks)
