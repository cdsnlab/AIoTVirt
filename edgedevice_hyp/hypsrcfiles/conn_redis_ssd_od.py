...
def main():
    ...
    th_listen.start()
    # make network connection to controller
    cntip, cntport = cntaddr.split('/',1)
    target_sock = message_bus.run_client(cntip, cntport)
    ...
    # sy: read video from file
        ...
        while (cap.isOpened()):
            ...
            if tr == 1:
                ...
                # send it directly
                #json_data_with_img = message_bus.create_message_numpy_img( frame )
                '''
                ...
                if tr == 2: # send detected frames
                    # yaya
                    print(tr)
                elif tr ==3: #send cropped
                    # yaya
                    print(tr)
                else: #send meta
                    # yaya
...
if __name__ == '__main__':
    ...
    parser.add_argument( '-tr', '--transmission', type=int,
                         default="1",
                         help="frame transmission options")
   
    parser.add_argument( '-cntaddr', '--controlleripport', type=str,
                         default="127.0.0.1/8888",
                         help="controller's ip (x) and port (y) in x/y format")

    parser.add_argument( '-camaddr', '--camipport', type=str,
                         default="127.0.0.1/44000/camera1",
                         help="this cam's ip (x),  port (y) and camera name (z) in x/y/z format")
    ...
    # is it live?
    live = ARGS.videofile
    # frame transmission option to controller 1: all, 2: upon detection (full frame), 3: upon detection (cropped frame), 4: meta data (text only) 
    tr = ARGS.transmission
    # controler ip and port
    cntaddr = ARGS.controlleripport
    camaddr = ARGS.camipport
