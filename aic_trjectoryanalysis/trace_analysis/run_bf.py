    ...
    for framenumber in range(1, max(MAXITER.items(), key=operator.itemgetter(1))[1]): # run through all gt files to find it at that framenumber. 
        ...
        for i in ids:
            ...
            if STATES[i]['cur'] == "NF":
                ...
                for item in items: 
                    if item[3] == i and item[2] == str(STATES[i]['camera']): #찾음.
                        STATES[i]['cur']="TRK"
        ...
        inputrow = {"scheme": "bf", "framenumber": framenumber, "ids": ids, "sector": SECTORTYPE, "initids": initids, "cur_states": states_merged, "reidcnts": allreidcnts, "camslookedat": allcamslookedat } 
        print(inputrow)
