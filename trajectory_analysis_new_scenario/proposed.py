...
mlmodel={}
print("**************************************")
def scenario_prop(vl=30, transitionmodel="conv_lstm", timemodel="ResNet", preprocessingmethod="last", sheetname="sheetname"):
    global mlmodel
    ...
    if transitionmodel == "conv_lstm":
        ...
        models = {cam: load_model('/home/spencer1/AIoTVirt/trajectoryanalysis/models/{}/pp_{}_vl_{}_cam_{}.h5'.format(transitionmodel, preprocessingmethod, vl, cam), compile=False) for cam in range(10)}

    elif transitionmodel == "svm" or transitionmodel == "rf" or transitionmodel == "dt":
        mlmodels_tr = {cam: load_model('/home/spencer1/AIoTVirt/trajectoryanalysis/joblibs_tr_dir/{}/pp_{}_vl_{}_cam_{}.h5'.format(transitionmodel, preprocessingmethod, vl, cam)) for cam in range(10)}

    if timemodel == "ResNet":
        #resnet_model = {cam: load_model('/home/boyan/AIoTVirt/trajectoryanalysis/models/mae_cam_last_resnet_1000_{}.h5'.format(cam), compile=False) for cam in range(10)}
        resnet_model = {cam: load_model('/home/spencer1/AIoTVirt/trajectoryanalysis/models/{}/pp_{}_vl_{}_cam_{}.h5'.format(timemodel, preprocessingmethod, vl, cam), compile=False) for cam in range(10)}
    elif timemodel == "rf" or timemodel == "dt" or timemodel == "svm":
        #mlmodel = {cam: joblib.load('/home/spencer1/AIoTVirt/trajectoryanalysis/joblibs/rf/{}_duration_transition_{}.joblib'.format(preprocessingmethod,cam))for cam in range(10)}
        mlmodel = {cam: joblib.load('/home/spencer1/AIoTVirt/trajectoryanalysis/joblibs/{}/pp_{}_vl_{}_cam_{}.joblib'.format(timemodel, preprocessingmethod,vl, cam)) for cam in range(10)}
    ...
    def hcf_based_handover_time(source, dest, cur_path, len_cur_path, angle_lim=30, cutoff_long=False, cutoff_short=False):
        remaining_durations = []
    ...
    def preprocess_track(track, vl, preprocessingmethod): #*ED
        # if len(track) < vl:
        #     continue
        # TODO NORMALIZE THIS FUCKING TRACK
        
        if preprocessingmethod == "ed":
            output = []
        ...
        elif preprocessingmethod == "last":
            output = []
            for x,y in track:
                output.append(np.array([x / 2560, y / 1440]))
            return np.array([output[-vl:]])
        '''
        elif preprocessingmethod == "sw-o":
            data=[]
            output=[]
            for x, y in track:
                data.append(np.array([x/2560, y/1440]))
            for i in range( len(track)-vl):
                if i >= len(track)-vl:
                    break
                output.append([data[i:i+vl]])
            return np.array(output)
            
        elif preprocessingmethod=="irw":
            ...
            #     output.append(np.array([x / 2560, y / 1440]))
            for prt in portions:
                startloc = int(len(track)/100*prt)
                for rw in rwset:
                    endloc=int(len(track)/100*rw)
                    btw = math.floor((endloc-startloc) / float(vl))
                    temp_value=[]
                    cnt=0
                    if(endloc-startloc)<vl:
                        continue
                    for x, y in track[startloc:endloc:btw]:
                        if cnt ==vl:
                            break
                        else:
                            temp_value.append(np.array([x / 2560, y / 1440]))
                        cnt+=1
                    output.append(temp_value)
            return output
        '''
    def model_based_handover_time(src_camera, full_track, vl=30, sample_method="last", timemodel="ResNet"):
        #global mlmodel
        
        if timemodel=="ResNet":
            if sample_method=="irw":
                track=preprocess_track(full_track, vl, "ed")
                estimation = resnet_model[src_camera].predict(track)
            elif sample_method=="sw-o":
                track=preprocess_track(full_track, vl, "last")
                estimation = resnet_model[src_camera].predict(track)
            else: # last, ed
                track=preprocess_track(full_track, vl, sample_method)
                estimation = resnet_model[src_camera].predict(track)

        elif timemodel=="dt" or timemodel =="rf" or timemodel == "svm":
            if sample_method=="irw":
                track=preprocess_track(full_track, vl, "ed")
                track = track.reshape(len(track), vl*2)
                estimation = int(mlmodel[src_camera].predict(track))
            elif sample_method=="sw-o":
                track=preprocess_track(full_track, vl, "last")
                track = track.reshape(len(track), vl*2)
                estimation = int(mlmodel[src_camera].predict(track))
    ...
    def estimate_transition():
        global passed_frames #, enable_filter, perform_handover, transition,  cam_tracking, 
        ...
        if passed_frames % 5 == 0:
            if recording[camera][-1]['duration'] > threshold:
                ...
                if preprocessingmethod == "irw":
                    track = preprocess_track(tracks, vl, "ed")
                    #print(track)
                elif preprocessingmethod == "sw-o":
                    track = preprocess_track(tracks, vl, "last")
                else: #last, ed
                    track = preprocess_track(tracks, vl, preprocessingmethod)
                ...
                for i, target in enumerate (choice):
                    ...
                    if target != -1:
                        if timemodel =="ResNet" or timemodel == "dt" or timemodel=="rf" or timemodel=="svm":
                            handover_td = model_based_handover_time(camera, full_track,vl, preprocessingmethod, timemodel)
                            handover_t, transition_d = handover_td, 0

                        elif timemodel=="hcf":
                            handover_t, transition_d = hcf_based_handover_time(camera, target, full_track, len_tracks, 10, False, False)
    ...
    if os.path.exists("/home/spencer1/AIoTVirt/trajectoryanalysis/results/scenario_newsimdata_prop_variation.xlsx"):
        with pd.ExcelWriter("/home/spencer1/AIoTVirt/trajectoryanalysis/results/scenario_newsimdata_prop_variation.xlsx", mode='a') as writer:
