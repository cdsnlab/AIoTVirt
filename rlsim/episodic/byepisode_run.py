...
# this program rewards system per episode not per frame.
...
def play():
    ...
    argparser.add_argument(
        ...
        metavar = 'w',
        default=0.9,
        help='weight btw gp, ee (def: 0.9). if large gp>ee priority.'
    )
    argparser.add_argument(
        '--scheme',
        metavar = 's',
    ...
    argparser.add_argument(
        ...
        metavar = 't',
        default=10,
        help='number of total iterations (def: 10).'
    )
    ...
    while iteration < totaliteration: 
        ...
        for i in range (0,int(agent[0].cap.get(cv2.CAP_PROP_FRAME_COUNT)-2)):
            ...
            # 3) reward
            env.step(env.action, count) # 여기서 input으로 previous reward 계산 필요.
    ...
    draw.singlegraphfromdict(gp_dict,dirn+fn_header,  "# iteration", "goodput (%)", "gp episode based")
    draw.singlegraphfromdict(ee_dict,dirn+fn_header, "# iteration", "energy usage (%)", "ee episode based")
