...
#vidpath = "/home/spencer/samplevideo/testedvids/6cam_rand_speed_" #longer version (60seconds)
class cam (object):
    ...
    def setupmodels(self, id):
        ...
        '''
        if id == "01": # use two different GPUs
            # use cuda device 1
            ...
            # use cuda device 2
            self.model = Darknet("cfg/2yolov3.cfg")
            self.model.load_weights('2yolov3.weights')
            self.cuda = torch.device('cuda:1')
            if torch.cuda.is_available():
                self.model.to(self.cuda)
                print("02_cuda set")
            self.model.eval()
    ...
    def loadvid(self, id):
        ...
        #self.vf = vidpath+id+"_2353_480_60.mp4"
        #self.vf = vidpath+id+"_2353_480_20.mp4"
        self.vf = vidpath+id+".avi"
